...
//         fetchOrders(currentPage, "Cancelled");
//       } else {
...
//   };
  ...
  // Use the useEffect hook to fetch orders when the page or filters change
  useEffect(() => {
    const status = "Cancelled"; // We're fetching cancelled orders
  ...
  const filterOrders = (orders) => {
    return orders.filter((order) => {
      ...
      const matchesCancelledStatus = order.status === "Cancelled"; // Filter by "Cancelled" status

      return matchesSearch && matchesAmount && matchesDate && matchesCancelledStatus;
    });
  };
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <FilterModal
        ...
        filterFields={[
          { name: "searchQuery", type: "text", placeholder: "Search by Order ID" },
          { name: "minAmount", type: "number", placeholder: "Min Amount" },
          { name: "maxAmount", type: "number", placeholder: "Max Amount" },
